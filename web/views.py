...
def solicitud_pedido(request):
    ...
    if request.user.is_authenticated and (request.user.is_staff or request.user.is_superuser):
        # Mostrar formulario deshabilitado y últimas solicitudes
        form = SolicitudPedidoForm()
        recientes = SolicitudPedido.objects.all().order_by('-created_at')[:20]
        messages.info(request, 'Los administradores no pueden solicitar pedidos. Revisa las solicitudes recientes.')
        return render(request, 'solicitud_pedido.html', {
            'form': form,
            'solicitudes_guardadas': recientes,
            'email_filter': None
        })
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                sol = SolicitudPedido.objects.create(
                    name=data.get('name'),
            ...
            try:
                ...
                try:
                    ...
                    comp_body = (
                        ...
                        f"• Solicitud #: {getattr(sol, 'pk', 'N/D')}\n"
                        f"• Producto: {producto_name}\n"
                        f"• Cantidad: {qty}\n"
                        f"• Total estimado: ${total_est}\n"
                        f"• Tamaño: {data.get('size','')}\n"
                        f"• Ocasión: {data.get('occasion','')}\n"
                        f"• Fecha del evento: {data.get('event_date','')}\n"
                    ...
                    logger.exception('No se pudo enviar el comprobante al cliente')
